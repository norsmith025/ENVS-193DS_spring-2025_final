...
ggplot(nest_boxes_clean, aes(x = edge_distance, y = sp, color = season)) +
  ...
  stat_smooth(method = "glm",
              ...
              fullrange = TRUE,
              aes(fill = season)) +
  scale_y_continuous(name = "Probability of Swift Parrot Nest Box Occupancy", limits = c(0, 1)) +
  scale_x_continuous(name = "Distance to the Forest Edge (m)") +
  theme_minimal() +
  theme(panel.grid = element_blank(), 
        panel.border = element_rect(color = "darkgrey", fill = NA)) +
  scale_color_manual(values = c("cadetblue4", "burlywood4")) +
  scale_fill_manual(values = c("cadetblue2", "bisque2")) +
  labs(title = "Swift Parrot Nest Box Occupancy Based on Model Predictions",
       color = "Season",
       fill = "Season")
```
