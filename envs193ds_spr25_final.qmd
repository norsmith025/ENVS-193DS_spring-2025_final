...
mod_predictions <- ggpredict(Model_2, terms = c("edge_distance", "season"))
...
ggplot() +
  ...
  geom_ribbon(data = mod_predictions,
              aes(x =x,
                  ymin = conf.low,
                  ymax = conf.high,
                  fill = group),
              alpha = 0.2) +
  geom_line(data = mod_predictions,
            aes(x =x,
                y = predicted,
                color = group),
            size = 1) +
  scale_y_continuous("Probability of Swift Parrot Nest Box Occupancy",
                     limits = c(0, 1),
                     breaks = seq(0, 1, 0.2)) + 
  theme_minimal() +
